        ...
        <div className="space-y-1">
          {conversations.length > 0 ? (
            conversations.map(conversation => {
              // Use the pre-built `user` object from the API response instead of
              // static mock lookup (findUserById only knows mock IDs, not DB ObjectIds)
              const otherUser = conversation.user;
              if (!otherUser) return null;

              // Determine the other participant's ID from participants array
              ...
              return (
                <div
                  ...
                  onClick={() => handleSelectUser(otherParticipantId)}
                >
                  <Avatar
